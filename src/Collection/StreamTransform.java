...
public class StreamTransform {
    ...
    public static void main(String[] args) {
        ...
        List<String> strs = Stream.of("Introduction", "to", "Java", "Programming").sorted().collect(toList());
        System.out.println("" + strs);

        List<Character> firstChars = strs.stream().map(s -> s.charAt(0)).collect(toList());
        System.out.println("first chars : " + firstChars);

        List<Double> results = Stream.generate(Math::random).peek(r -> System.out.println("Fetching " + r)).limit(10).collect(toList());
