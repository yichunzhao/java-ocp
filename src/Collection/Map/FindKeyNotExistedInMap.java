...
/**
 * As finding a key-value that is not existed in a map, it returns a null; however, in some cases it may cause
 * NullPointException when assigning it to a primitive variable.
 * <p>
 * Being safe: using map method: computeIfPresent(key, (key, value)-> value);
 */
public class FindKeyNotExistedInMap {

    public static void main(String[] args) {

        Map<String, Integer> wordLengthMap = Arrays.stream("Java is a popular programing language"
                .split("\\s"))
        ...
        System.out.printf("compute k: xxx if it is present: %s \n", wordLengthMap.computeIfPresent("xxx", (k, v) -> v));
        System.out.printf("compute k: Java if it is present: %s \n", wordLengthMap.computeIfPresent("Java", (k, v) -> v += 1));
    }
