...
public class ShowAllZoneID {
  ...
  public static void main(String[] args) {
    ...
    Map<ZoneOffset, List<ZonedDateTime>> zoneOffsetListMap =
        ZoneId.getAvailableZoneIds().stream()
    ...
    zoneOffsetListMap.forEach(
        (zoneOffset, zonedDateTimes) ->
            System.out.println(
                zoneOffset
                    + "\n"
                    + zonedDateTimes.stream()
                        .map(z -> z.getZone().getId())
                        .collect(Collectors.joining(" & "))));

    System.out.println(
        "#################### ZoneOffSet map to its Count(ZoneIds) ######################");
    ZoneId.getAvailableZoneIds().stream()
        .map(s -> ZonedDateTime.now(ZoneId.of(s)))
        .collect(
            Collectors.groupingBy(
                zonedDateTime -> zonedDateTime.getOffset(), Collectors.counting()))
