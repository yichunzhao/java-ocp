 ...
 *
 * <p>where to define nested classes? within a scope of a class. TreeSort and Tree survive together;
 * without a Tree we don't need a TreeSort, that is the reason we use inner class here. Another case
 * to use nested class, is to take it as parameter containers, so that reducing the length of method
 * due to many arguments.
 *
 * <p>inner class cannot have static methods. static nested class can have static methods.
 *
 * <p>Static class member need not to create a outer class instance to access it. A nested static
 * class is compiled as separated class file.
 */
public class Tree {
