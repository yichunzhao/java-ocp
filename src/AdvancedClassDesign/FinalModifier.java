...
/**
 * The final keyword is a non-access modifier, it can modify classes, methods and variables; the
 * semantics of final it means it cannot be changed once it is initialized or created.
 *
 * <p>final class can not be extended; final method cannot be overridden.
 *
 * <p>final variable can be init once; once it is initialized then it cannot be modified.
 *
 * <p>final variables must be init. or generating compilation errors.
 *
 * <p>static final var. cannot be init. in an instance constructor. So it must be init as declared,
 * or with a static block. instance final var. can be init. via an instance constructor.
 *
 * <p>You can assign a static final variable via a static method, because a static method can be
 * invoked more than once. That is the reason why it generates a compilation error.
 */
public class FinalModifier {
  ...
  // if you added a non-arg constructor, it generates a compilation error; for the static instance
  // var may not
  // be init.
  ...
  public void setInstanceMethodArg(final StringBuilder sb) {
    ...
    sb.append("another");
    // however its instance reference cannot be modified, meaning that you can point it to another
    // instance.
