 ...
 *
 * <p>p.resolve(p1), it read as path p resolve path p1 within the path context of p. it means path p
 * join path p1 based on its path.
 *
 * <p>So you need to watch out if making sense as joining two paths. for instance, a relative path
 * resolves an absolute path doesn't any sense, because an absolute path doesn't depend on any other
 * path, so the java will return the absolute path again.
 *
 * <p>resolve(String) and resolve(Path) is used to join a relative path to another path.
 *
 * <p>what happens when resolve(null)? overloading methods are confused, so compiler throw compiling
 * error.
 *
 * <p>the resolve action doesn't work on the real file system;
 */
