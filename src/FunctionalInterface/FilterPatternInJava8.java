...
public class FilterPatternInJava8 {
    ...
    public static void main(String[] args) {
        List<User> users = Stream.of(new User("Yichun Zhao", 36),
                new User("Mike xxx", 32), new User("Ludovic Dewailly", 30),
                new User("Daoqi Yang", 56))
                .collect(toList());
        System.out.println("before filtering" + users);

        Criteria<User> criteriaByAge_32 = (us) -> {
            return us.stream().filter(u -> u.age() > 32).collect(toList());
        };

        Criteria<User> criteriaByAge_30 = (us) -> {
