...
public class SortingMapByComparator {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Integer> map = strings.stream()
                .collect(Collectors.toMap(s -> s, s -> s.length(), (o, n) -> n));
        System.out.println("string-length-map: " + map);

        //sorting by word length without overlapping
        Comparator<Map.Entry<String, Integer>> comparator = Comparator.comparing(Map.Entry::getValue);
        List<Map.Entry<String, Integer>> sortedMap = map.entrySet().stream().sorted(comparator.reversed()).collect(Collectors.toList());
