...
public class StreamGenerateMethod {
    ...
    public static void main(String[] args) {
        ...
        List<Double> doubleNumbers = Stream.generate(() -> Math.random()).limit(10).sorted().collect(toList());
        System.out.println("doubleNumbers: " + doubleNumbers);

        Optional<Double> maxValuedoubleNumbers = doubleNumbers.stream().max((d1, d2) -> d1.compareTo(d2));
        System.out.println("max value among 10: " + maxValuedoubleNumbers.orElse(Double.NaN));
        
        Optional<Double> minValueDoubleNumbers = doubleNumbers.stream().min((d1,d2)-> d1.compareTo(d2));
