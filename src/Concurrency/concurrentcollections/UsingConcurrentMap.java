...
/**
 * A concrete implementation of the ConcurrentMap interface, class ConcurrentHashMap is a concurrent
 * class analogous to class HashMap.
 *
 * <p>Operations on a ConcurrentMap are made atomic, so as to avoid synchronisation.
 */
  ...
  */
  static void manipulateMap(String item, Integer amount) {
    if (itemAmountMap.containsKey(item)) // read operation
    {
      ...
      System.out.printf(
          "current thread %s: item = %s; amount=%d \n", currentThreadName, item, amount);
