 ...
 *
 * <p>A big task is divided into tasks small enough to handle and assemble results afterwards.
 *
 * <p>A ForkJoinPool consume a ForkJoinTask, which is an Abstract class. It has two sub-classes,
 * i.e. RecursiveTask and RecursiveAction.
 *
 * <p>A ForkJoinPool extends ExecutorService interface, so it follows Executor design concept,
 * consisting of a task queue and a thread pool. Each worker thread within a ForkJoinPool has its
 * own Deque to store the tasks; and it allows free workers to steal the task from other busy
 * workers.
 */
