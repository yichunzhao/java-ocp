...
/**
 * wildcard type ? means could be any type of data. However, the compiler setup a protection here;
 * you cannot aad an element inside the list instance, because it is not sure what you will put
 * inside it.
 *
 * <p>you can read from a List<?>, however if you try to modify a List<?>, you will get an error:
 * "capture of ?"
 *
 * <p>so, List<?> is readable but not writable.
 *
 * <p>Then why designing this mechanism, I think it is only for transferring different parameterised
 * types into a method argument, meanwhile inside the method it cannot be modified.
 */
public class WildCardGenericType {
  ...
  public static List<?> receiveDiverseItems(List<?> items) {
    ...
    /**
     * List<?> cannot be modified inside the method, because the compiler don't exactly know what
     * type inside this collection.
     */
