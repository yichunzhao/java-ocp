...
/**
 * to use methods format() and printf(), you need to define a format string that defines how to
 * format text and an object argument list that defines what to format.
 *
 * <p>format specifiers A format specification must start with a % sign and end with conversion
 * character %[argument_index$][flags][width][.precision]conversion_char
 *
 * <p>argument_index: optional decimal integer indicating the position of the argument in the
 * argument list. the first argument is referred by 1$, the second by 2$, and so forth.
 *
 * <p>conversion_char: required. The conversion characters on the exam are %b, %c, %d, %f ,and %s A
 * character indicating how the argument should be formatted. The set of valid conversions for a
 * given argument depends on the argument's data type.
 *
 * <p>You can pass any type of primitive variable or object reference to %b.
 */
